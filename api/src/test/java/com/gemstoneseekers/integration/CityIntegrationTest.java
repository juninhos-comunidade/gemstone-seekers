...
class CityIntegrationTest {
    ...
    @Test
    void shouldReturnCitiesWithJwt() {
        String accessToken = getAccessToken();
        given().contentType(ContentType.JSON).header("Authorization", "Bearer " + accessToken).when().get(
                "/api/v1/cities").then().statusCode(200).body("success", equalTo(true)).body("result", notNullValue())
    ...
    void shouldReturnCitiesByStateId() {
        String accessToken = getAccessToken();
        given().contentType(ContentType.JSON).header("Authorization", "Bearer " + accessToken).when().get(
                "/api/v1/states/1/cities").then().statusCode(200).body("success", equalTo(true)).body("result",
