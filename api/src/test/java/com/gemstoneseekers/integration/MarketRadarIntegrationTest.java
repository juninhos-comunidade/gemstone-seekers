...
class MarketRadarIntegrationTest {
    ...
    void shouldRejectWithoutJwt() {
        given().contentType(ContentType.JSON).when().get("/api/v1/market-radar/technology-demand").then().statusCode(
                401).body("success", equalTo(false)).body("error.code", equalTo("UNAUTHENTICATED"));
    }

    @Test
    void shouldReturnEmptyListWhenNoJobsExist() {
        String token = getAccessToken();
        given().contentType(ContentType.JSON).header("Authorization", "Bearer " + token).when().get(
                "/api/v1/market-radar/technology-demand").then().statusCode(200).body("success", equalTo(true)).body(
                        "result", hasSize(0));
    }
    ...
    void shouldReturnTechnologyDemandForOpenJobsOnly() throws SQLException {
        ...
        given().contentType(ContentType.JSON).header("Authorization", "Bearer " + token).when().get(
                "/api/v1/market-radar/technology-demand").then().statusCode(200).body("success", equalTo(true)).body(
                        "result", hasSize(2)).body("result[0].technologyName", equalTo("Java")).body(
                                "result[0].jobCount", equalTo(2)).body("result[0].mandatoryCount", equalTo(1)).body(
                                        "result[1].technologyName", equalTo("Python")).body("result[1].jobCount",
                                                equalTo(1)).body("result[1].mandatoryCount", equalTo(1));
    }
}
