...
class StateIntegrationTest {
    ...
    void shouldReturnStatesWithJwt() {
        ...
        given().contentType(ContentType.JSON).header("Authorization", "Bearer " + accessToken).when().get(
                "/api/v1/states").then().statusCode(200).body("success", equalTo(true)).body("result", notNullValue())
    ...
    void shouldReturnStatesByCountryId() {
        String accessToken = getAccessToken();
        given().contentType(ContentType.JSON).header("Authorization", "Bearer " + accessToken).when().get(
                "/api/v1/countries/1/states").then().statusCode(200).body("success", equalTo(true)).body("result",
