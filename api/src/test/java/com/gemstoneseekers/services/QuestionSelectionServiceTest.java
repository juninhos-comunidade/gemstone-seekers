...
class QuestionSelectionServiceTest {
    ...
    void shouldReturnTenUnseenQuestionsWhenStockIsSufficient() {
        ...
        when(questionRepository.findRandomUnseenQuestions(user.getId(), technology.getId(), QuestionDifficulty.INTERMEDIATE.name(), 10))
    ...
    void shouldThrowExceptionAndPublishEventWhenUnseenQuestionsAreLessThanThree() {
        ...
        when(questionRepository.findRandomUnseenQuestions(user.getId(), technology.getId(), QuestionDifficulty.ADVANCED.name(), 10))
            .thenReturn(mockQuestions);
    ...
    void shouldFetchSeenQuestionsAndPublishEventWhenUnseenQuestionsAreLessThanSix() {
        ...
        when(questionRepository.findRandomUnseenQuestions(user.getId(), technology.getId(), QuestionDifficulty.BEGINNER.name(), 10))
            .thenReturn(unseenMockQuestions);
        when(questionRepository.findRandomSeenQuestions(user.getId(), technology.getId(), QuestionDifficulty.BEGINNER.name(), 6))
            .thenReturn(seenMockQuestions);
    ...
    void shouldThrowExceptionAndPublishEventTwiceWhenSeenQuestionsAreInsufficient() {
        ...
        when(questionRepository.findRandomUnseenQuestions(user.getId(), technology.getId(), QuestionDifficulty.INTERMEDIATE.name(), 10))
            .thenReturn(unseenMockQuestions);
        when(questionRepository.findRandomSeenQuestions(user.getId(), technology.getId(), QuestionDifficulty.INTERMEDIATE.name(), 6))
            .thenReturn(seenMockQuestions);
    ...
    void shouldFetchSeenQuestionsSilentlyWhenUnseenQuestionsAreBetweenSixAndNine() {
        ...
        when(questionRepository.findRandomUnseenQuestions(user.getId(), technology.getId(), QuestionDifficulty.ADVANCED.name(), 10))
            .thenReturn(unseenMockQuestions);
        when(questionRepository.findRandomSeenQuestions(user.getId(), technology.getId(), QuestionDifficulty.ADVANCED.name(), 2))
            .thenReturn(seenMockQuestions);
